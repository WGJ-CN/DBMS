...
void database::backup(const char *backup_dir)
{
	if(!is_opened())
	{
		std::fprintf(stderr, "[Error] Backup: database not opened!\n");
		return;
	}

	// Close database to ensure all data is written to disk
	close();
	...
	else
	{
        ...
        backup_path = "./database/backup_" + std::string(info.db_name) + "_" + timestamp;
	}

	// Create backup directory
	#ifdef _WIN32
	if(mkdir(backup_path.c_str()) != 0)
	#else
	if(mkdir(backup_path.c_str(), 0755) != 0)
	#endif
	{
		std::fprintf(stderr, "[Error] Backup: failed to create backup directory!\n");
		// Reopen database
		open(info.db_name);
		return;
	}
	...
	std::string backup_db_file = backup_path + "/" + info.db_name + ".database";
	if(std::ifstream(db_file, std::ios::binary).read(nullptr, 0).fail())
	{
		std::fprintf(stderr, "[Error] Backup: database file not found!\n");
		// Reopen database
		open(info.db_name);
		return;
	}
	
	// Copy database info file
	{
		std::ifstream src(db_file, std::ios::binary);
		std::ofstream dst(backup_db_file, std::ios::binary);
		dst << src.rdbuf();
	}
	...
	for(int i = 0; i < info.table_num; ++i)
	{
		...
		std::string table_name = info.table_name[i];
		std::string table_head_file = get_db_file_path(table_name.c_str()) + ".thead";
		std::string backup_table_head_file = backup_path + "/" + table_name + ".thead";
		
		std::ifstream src_head(table_head_file, std::ios::binary);
		std::ofstream dst_head(backup_table_head_file, std::ios::binary);
		dst_head << src_head.rdbuf();

		// Copy table data file
		std::string table_data_file = get_db_file_path(table_name.c_str()) + ".tdata";
		std::string backup_table_data_file = backup_path + "/" + table_name + ".tdata";
		
		std::ifstream src_data(table_data_file, std::ios::binary);
		std::ofstream dst_data(backup_table_data_file, std::ios::binary);
		dst_data << src_data.rdbuf();
	}

	// Reopen database
	open(info.db_name);
	std::printf("[Info] Backup completed successfully! Backup path: %s\n", backup_path.c_str());
}

void database::restore(const char *backup_dir)
{
	if(is_opened())
	{
		std::fprintf(stderr, "[Error] Restore: please close current database first!\n");
		return;
	}
	...
	#ifdef _WIN32
	struct stat st;
	if(stat(backup_dir, &st) != 0 || !(st.st_mode & S_IFDIR))
	#else
	struct stat st;
	if(stat(backup_dir, &st) != 0 || !S_ISDIR(st.st_mode))
	#endif
	{
		std::fprintf(stderr, "[Error] Restore: backup directory not found!\n");
		return;
	}

	// Read database info from backup
	database_info backup_info;
	std::string backup_db_file = std::string(backup_dir) + "/" + "*.database";
	WIN32_FIND_DATA find_data;
	HANDLE hFind = FindFirstFile(backup_db_file.c_str(), &find_data);
	if(hFind == INVALID_HANDLE_VALUE)
	{
		std::fprintf(stderr, "[Error] Restore: no database file found in backup!\n");
		return;
	}

	std::string db_name = find_data.cFileName;
	// Remove .database extension
	db_name = db_name.substr(0, db_name.size() - 9); // 9 is the length of ".database"
	...
	std::string current_db_file = get_db_file_path(db_name.c_str()) + ".database";
	std::remove(current_db_file.c_str());

	// Copy database file from backup
	backup_db_file = std::string(backup_dir) + "/" + db_name + ".database";
	{
		std::ifstream src(backup_db_file, std::ios::binary);
		std::ofstream dst(current_db_file, std::ios::binary);
		dst << src.rdbuf();
	}

	// Read database info
	std::ifstream ifs(current_db_file, std::ios::binary);
	ifs.read((char*)&info, sizeof(info));
	ifs.close();
	...
	for(int i = 0; i < info.table_num; ++i)
	{
		...
		// Remove existing table files if they exist
		std::string table_head_file = get_db_file_path(table_name.c_str()) + ".thead";
		std::string table_data_file = get_db_file_path(table_name.c_str()) + ".tdata";
		std::remove(table_head_file.c_str());
		std::remove(table_data_file.c_str());

		// Copy table header file from backup
		std::string backup_table_head_file = std::string(backup_dir) + "/" + table_name + ".thead";
		{
			std::ifstream src_head(backup_table_head_file, std::ios::binary);
			std::ofstream dst_head(table_head_file, std::ios::binary);
			dst_head << src_head.rdbuf();
		}

		// Copy table data file from backup
		std::string backup_table_data_file = std::string(backup_dir) + "/" + table_name + ".tdata";
		{
			std::ifstream src_data(backup_table_data_file, std::ios::binary);
			std::ofstream dst_data(table_data_file, std::ios::binary);
			dst_data << src_data.rdbuf();
		}
	}
	...
	open(db_name.c_str());
	std::printf("[Info] Restore completed successfully! Database '%s' restored.\n", db_name.c_str());
}
