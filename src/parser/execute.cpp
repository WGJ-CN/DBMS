...
void execute_backup(const char *backup_file) {
    // 移除字符串引号
    if (backup_file[0] == '"' && backup_file[strlen(backup_file) - 1] == '"') {
        char *clean_file = strdup(backup_file + 1);
        clean_file[strlen(clean_file) - 1] = '\0';
        dbms::get_instance()->backup(clean_file);
        free(clean_file);
    } else {
        dbms::get_instance()->backup(backup_file);
    }
    free((char*)backup_file);
}

void execute_restore(const char *backup_file) {
    // 移除字符串引号
    if (backup_file[0] == '"' && backup_file[strlen(backup_file) - 1] == '"') {
        char *clean_file = strdup(backup_file + 1);
        clean_file[strlen(clean_file) - 1] = '\0';
        dbms::get_instance()->restore(clean_file);
        free(clean_file);
    } else {
        dbms::get_instance()->restore(backup_file);
    }
    free((char*)backup_file);
}
