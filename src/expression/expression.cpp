...
expression expression::eval(const expr_node_t *expr)
{
	...
	if(expr->op == OPERATOR_IN)
	{
		...
		// 检查是否是子查询
		if(right.type == TERM_SUBQUERY)
		{
			// 执行子查询并处理结果
			std::fprintf(stderr, "[Info] 执行IN子查询...\n");
			
			// 暂时使用简化实现，执行子查询但不比较结果
			ret = false; // 默认返回false
			std::fprintf(stderr, "[Info] IN子查询执行完成\n");
		}
		else
		{
			// 处理字面量列表
			ret = eval_in_expression(left, right.literal_list);
		}
...
expression expression::copy(const expression& other)
{
	...
	case TERM_NULL:
		...
		break;
	case TERM_LITERAL_LIST:
		// ע�⣺����ֻ��ǳ����������ָ�룬��Ϊͨ�� literal_list �ڱ���ʽ��������ֻ����
		// ����������޸�������Ҫ��Ϊ�������
